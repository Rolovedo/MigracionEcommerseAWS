...
<v-product-ratings
    {{ $attributes->merge(['class' => 'flex w-max items-center rounded-md border border-brown-200 px-4 py-2']) }}
    average="{{ $average }}"
    total="{{ $total }}"
>
</v-product-ratings>
...
@pushOnce("scripts")
    ...
    >
        <div>
            ...
            <span class="border-l border-brown-300 text-sm font-medium text-black max-sm:border-brown-300 max-sm:text-xs ltr:pl-1 rtl:pr-1">
                @{{ abbreviatedTotal }}
